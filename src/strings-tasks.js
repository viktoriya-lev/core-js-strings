...
function removeFirstOccurrences(str, value) {
  ...
  const firstOccurenceIndex = str.indexOf(value);
  const occurenceRange = firstOccurenceIndex + value.length;

  const str1 = str.slice(0, firstOccurenceIndex);
  const str2 = str.slice(occurenceRange);
...
function removeLastOccurrences(str, value) {
  ...
  const firstOccurenceIndex = str.lastIndexOf(value);
  const occurenceRange = firstOccurenceIndex + value.length;
...
function formatTime(minutes, seconds) {
  ...
  return result.concat(min.padStart(2, '0'), ':', sec.padStart(2, '0'));
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < splittedString.length; i += 1) {
    if (vowelsArr.includes(splittedString[i])) {
      counter += 1;
    }
  }
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < charArr.length; i += 1) {
    if (!punctuationMarks.includes(charArr[i])) {
      firstArr += charArr[i];
    }
  }
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < wordsArr.length; i += 1) {
    longestWord =
      wordsArr[i].length > longestWord.length ? wordsArr[i] : longestWord;
  }
...
function extractNameFromTemplate(value) {
  const strArr = value.split(' ').splice(1);

  const nameArr = strArr.map((item) => {
    let newItem = item;
...
function extractEmails(str) {
  ...
  strToArr.forEach((item) => emailArr.push(item));
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (punctuationMarks.includes(str[i])) {
      ...
      if (
        ...
        encryptedStr += String.fromCharCode(charCode + 13);
      } else {
        encryptedStr += String.fromCharCode(charCode - 13);
      }
    }
